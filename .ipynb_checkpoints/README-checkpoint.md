...
The app returns a simple diagnosis (0: Pneumonia, 1: No Pneumonia but something else, 3: Normal) and their respective predicted probabilities. The webapp can be run locally and is entitled `app.py` in the `code` folder of this repository. Streamlit is required to run the app. Please download the necessary trained models before instantiating the webapp. 
